...
3. Dar clic en la pestaña ```PUERTOS``` y localizar el puerto ```desktop(6080)```.
4. En la columna ```Dirección reenviada``` dar clic en el Globo terráqueo para abrir una pestaña del navegador donde estará el *desktop-lite*. 
5. En la nueva pestaña abierta se mostrará el mensaje *noVNC* y dar clic en el botón ```Conectar```.
6. Capturar la contraseña ```vscode``` y dar clic en el botón ```Aceptar```.
7. Se mostrará un escritorio simple con una barra de tarea inferior y un menu de tareas en la esquina inferior izquierda.
